...
### 近期
- [x] `/init/2/2` — 同步已有云仓库（clone 远程仓库到本地）
- [x] `/init/2/3` — 选择已有本地仓库（关联已存在的 Git 仓库）
- [x] `/init/2/4` — 创建本地仓库，暂不同步（纯本地模式）
- [x] **主页开发** — 笔记管理界面（详见下方设计）
- [x] 笔记 CRUD — 创建、编辑、删除笔记
- [ ] **自动同步** — 启动 pull + 保存 push（详见下方设计）
...
参考常见笔记应用（Notion、Obsidian）和邮箱客户端的经典三栏布局：
...
#### 1. 顶部导航栏
- **左上角**：GitNoteLine 品牌标识
- **右上角**：设置入口（未来扩展）

#### 2. 左下角：仓库选择器（已优化）
- **只显示仓库名称**（从 repositories 表读取 name 或从路径提取）
- **位置调整到底部**，而不是顶部
- **边框淡化**，减少视觉噪音
- 倒三角图标，点击展开下拉菜单
- **当前版本**：只显示一个仓库（初始化时设置的）
- **未来扩展**：支持下拉选择多个仓库（需要修改数据库支持多仓库）

#### 3. 左侧：笔记列表（已优化）
- 显示当前仓库中的所有 `.md` 文件
- 按文件名排序（或按修改时间？）
- 点击笔记名称 → 右侧**只读预览**（渲染 Markdown）
- **底部加号按钮（已优化）**：
  - **不再使用大按钮**，改用小巧的图标
  - **用竖线与列表分隔**，而不是独立按钮
  - **无底色、无边框**，保持视觉简洁
  - 点击弹出新建笔记对话框
  - 输入框：笔记名称（自动追加 `.md` 后缀显示）
  - 确认创建 → 在仓库根目录创建 `{name}.md` 文件
  - **冲突处理**：如果文件已存在，显示错误提示
  - **创建成功后，自动跳转到编辑器页面**（刚创建肯定想写内容）

#### 4. 右侧：笔记预览/查看器（已实现）
- 点击列表中的笔记 → 右侧显示**只读预览**（用 marked.js 渲染 Markdown）
- 预览区顶部显示笔记标题 + 编辑按钮（pen-to-square 图标）
- 点击编辑按钮 → 跳转到 `/editor` 页面进入完整编辑模式
- 未选中笔记时显示占位提示

**交互流程：**
```
列表点击 → 只读预览（轻量快速浏览）
预览区编辑图标 → 跳转 /editor（完整编辑）
新建笔记确认 → 直接跳转 /editor（刚创建即编辑）
```
...
#### 后端 API（需要新增）
```
GET  /api/notes/list          # 获取笔记列表（读取仓库中的 .md 文件）
GET  /api/notes/<name>        # 获取指定笔记内容
POST /api/notes               # 创建新笔记（写入文件）
...
│   └── main.js        # 主页交互逻辑
└── css/
    └── style.css      # 添加主页样式（三栏布局）
```

#### 数据流
1. 页面加载 → 调用 `/api/notes/list` 获取笔记列表
2. 点击笔记 → 调用 `/api/notes/<name>` 获取内容 → 显示在右侧
3. 点击加号 → 弹出对话框 → 输入名称 → 调用 `POST /api/notes` → 刷新列表
4. 编辑笔记 → 调用 `PUT /api/notes/<name>` → 可选自动 commit

### 待确认的设计问题

1. **笔记组织**：✅ **支持子目录/文件夹** — 笔记可以组织在子目录中
2. **排序方式**：✅ **按文件名排序** — 字母顺序排列
3. **自动保存**：待确认（Phase 2 再决定）
4. **Git 集成**：✅ **Phase 1 不做 commit/push** — 先完成笔记界面，后续再加 Git 操作
5. **笔记删除**：待确认（Phase 1 先实现基础删除）
6. **多仓库切换**：待确认（当前单仓库）
...
**Phase 1（最小可用版本）— 当前重点：**
- [ ] 三栏布局 HTML/CSS
- [ ] 笔记列表显示（读取仓库中的 .md 文件，**支持子目录**）
- [ ] **按文件名排序**
- [ ] 新建笔记（创建文件，**支持指定子目录路径**）
- [ ] 笔记内容查看（只读）
- [ ] **不做 Git commit/push** — 纯文件操作
...
**默认自动模式：**
- 应用启动 → 自动 `git pull`
- 每次保存笔记 → 更新 `.gitnoteline.yaml` 的 `last_sync` → 自动 `git commit` → 自动 `git push`

**可选手动模式（设置中切换）：**
- 用户点击同步图标手动触发 pull/push

### 状态反馈

**同步图标（arrows-rotate）位置：** 顶部导航栏右侧（设置按钮旁边）

| 状态 | 图标表现 | 交互 |
|------|---------|------|
| 同步中 | 旋转动画 | — |
| 成功 | 默认色 | — |
| 失败 | 红色 | hover 显示错误详情 tooltip |

### 错误处理

| 场景 | 处理方式 |
|------|---------|
| 启动 pull 网络失败 | 图标变红，tooltip 显示"无法连接远程仓库" |
| 保存 push 网络失败 | 图标变红，tooltip 显示"推送失败"，下次保存时重试 |
| 合并冲突 | 自动 `git merge` 后重新推送 |
| 认证失败 | 图标变红，tooltip 显示"认证失败，请检查凭证" |

### 后端 API

```
POST /api/repo/<id>/pull     # 拉取远程更新
POST /api/repo/<id>/push     # 提交并推送
GET  /api/repo/<id>/sync-status  # 获取同步状态
```
