/**
 * init-2-2.js — Clone existing remote repository.
 */

;(function () {
  'use strict'

  const form = document.getElementById('form-step2-2')
  ...
  async function fetchDefaultPath(remoteUrl) {
    try {
      const res = await fetch(`/api/init/step2/2/default-path?remote_url=${encodeURIComponent(remoteUrl)}`)
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!remoteUrl) {
      showError('远程仓库 URL 不能为空')
      remoteUrlField.focus()
      return
    }
    if (!localPath) {
      showError('本地路径不能为空')
    ...
    try {
      const res = await fetch('/api/init/step2/2', {
        method: 'POST',
